...
工具的每个字符串参数在操作前检查是否写死了配置中的样例值，ref 定位取得的描述符也会检查。固定输入被拒绝后，必须用正确参数引用重新执行该类操作，不能仅靠后续断言中的占位符结束步骤。该检查采取保守策略：静态文案恰好包含样例值时，也可能要求改写；请使用具有区分度的样例值和显式规则，不要把过于通用的短文本作为参数样例。

规则范围、数量和可见性等待共享一个截止时间，不分别获得完整等待窗口。当前步骤未完成会丢弃当前会话，通过 `--max-repairs` 从新会话重探整个流程；不会在已改变的 UI 状态上重复该步骤。新会话仍不自动清空 Electron 的磁盘缓存。
