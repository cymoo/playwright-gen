...
> **状态（已落地 / 已修正）**：本方案的方向已实现，但版本被重新组织为按**页面复杂度**演进的 v0→v3
> （见根目录 `README.md` 与 `AGENTS.md`）。落地时做了几处修正：
> ① 本文推迟到"未来"的**轨迹→代码**提前到 v2（最大质量杠杆）；
> ② 元素定位从"文本包含"升级为 **ARIA 无障碍树 + ref 寻址**；
> ③ 本文 `BrowserSession` 用同步 Playwright 在 lovia 的 async 循环里会出错 → 改 **async**；
> ④ 本文"Agent 调 screenshot 让视觉模型看图"在本栈行不通（lovia 工具返回纯文本、DeepSeek 无视觉）
> → 改为 **Qwen 视觉工具返回文本**；
> ⑤ 断言升级为一等公民（assert_* 工具实时校验且记入轨迹）。
> 本文以下内容保留作设计存档。
